...
new IframeIoIframe({
  onSetData: data => {
    const result = JSON.parse(data && typeof data === 'string' ? JSON.parse(data) : JSON.stringify({}))
    const { editCanvasConfig, requestGlobalConfig, componentList } = result
    if(componentList.length>0){
      chartEditStore.editCanvasConfig = editCanvasConfig || {}
      chartEditStore.requestGlobalConfig = requestGlobalConfig || {}
      chartEditStore.componentList = componentList || []
      useComInstall(chartEditStore as any)

    }
    console.log(chartEditStore.componentList, 'chartEditStore')
  },
  onRequestGetData: async params => {
    console.log('onRequestGetData')
    console.log(params, '收到获取iframe的数据请求了，请返回数据')
    const data = await chartEditStore.getStorageInfo()
